...
python3 -c "import sys;import json;print(json.loads(json.loads(input())['SecretString'])['private'])" <<< $(aws secretsmanager get-secret-value --secret-id web-app --region eu-central-1) > ssh_tmp
